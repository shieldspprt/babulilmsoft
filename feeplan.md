...
| Old (v3 — Bills) | New (v4 — Subscription) |
|-------------------|------------------------|
| 2 tables (fee_bills + fee_payments) | 1 table (fee_payments only) |
| Bill generation on view | No generation — pure calculation |
| Bill snapshots (children_data JSONB) | No snapshots — always live data |
| Carry-forward stored in bill rows | Carry-forward computed in memory |
| Delete payment → cascade issues | Delete payment → auto-recalculate |
| Per-month status stored in DB | Status derived from payment history |
| Complex allocation algorithm | Simple: total owed vs total paid |
| Multiple DB operations per payment | 1 INSERT per payment |
