...
export const Dashboard = () => {
  ...
  const checkCreditStatus = () => {
    ...
    if (expired || profile.total_credits <= 0) {
      setActiveTab('buy');
    }
  };
            ...
            <div className="overview-grid">
              <div className={getStatCardClass()}>
                <div className="stat-icon">
                  {creditExpired ? <AlertTriangle size={32} /> : <Wallet size={32} />}
                </div>
                <div className="stat-info">
                  <span className="stat-label">
                    {creditExpired ? 'Account Expired' : 'Available Balance'}
                  </span>
                  <span className={`stat-value ${creditExpired ? 'expired' : ''}`}>
                    {creditExpired ? '0' : profile.total_credits} Credits
                  </span>
                  <span className={`stat-sub ${creditExpired ? 'expired' : daysRemaining <= 7 ? 'warning' : ''}`}>
                    {creditExpired 
                      ? 'Purchase credits to reactivate' 
                      : `${daysRemaining} Days remaining`}
                  </span>
                  {profile.credit_expires_at && !creditExpired && (
                    <span className="stat-sub">
                      Expires: {new Date(profile.credit_expires_at).toLocaleDateString()}
                    </span>
                  )}
                </div>
              </div>
              <div className="info-card glass">
                ...
                <div className="card-actions mt-4">
                  <Button onClick={() => setActiveTab('buy')}>
                    {creditExpired ? 'Reactivate Account' : 'Recharge Now'}
                  </Button>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'buy' && (
            <div className="buy-section">
              {creditExpired && (
                <div className="expiry-notice glass">
                  ...
                  <div>
                    ...
                    <p>Your account has expired. Select a plan below to reactivate your access.</p>
                  </div>
                </div>
              )}
              ...
              <div className="pricing-grid">
                <div 
                  className={`pricing-card glass ${selectedPlan?.credits === 30 ? 'selected' : ''}`} 
                ...
                >
                  ...
                  <div className="plan-per">1 credit = 1 day</div>
                ...
                <div 
                  className={`pricing-card glass border-accent ${selectedPlan?.credits === 100 ? 'selected' : ''}`} 
                ...
                <div className="checkout-card glass animate-fade-in">
                  ...
                  <div className="purchase-summary">
                    <div className="summary-row">
                      <span>Plan</span>
                      <span>{selectedPlan.credits} Credits</span>
                    </div>
                    <div className="summary-row">
                      <span>Duration</span>
                      <span>{selectedPlan.credits} Days</span>
                    </div>
                    <div className="summary-row total">
                      <span>Total</span>
                      <span>Rs {selectedPlan.pkr.toLocaleString()}</span>
                    </div>
                  </div>

                  <div className="payment-methods">
                    <button 
                      className={`pay-btn ${paymentMethod === 'JazzCash' ? 'active' : ''}`} 
                  ...
                  <div className="payment-instructions">
                    {paymentMethod === 'JazzCash' ? (
                      <>
                        <p>Send <strong>Rs {selectedPlan.pkr.toLocaleString()}</strong> via JazzCash to:</p>
                        <div className="account-box">0300-1234567 (ilmsoft)</div>
                      </>
                    ) : (
                      <>
                        <p>Transfer <strong>Rs {selectedPlan.pkr.toLocaleString()}</strong> to Bank Account:</p>
                    ...
                    <p className="instruction-note">
                      After payment, enter your transaction reference below.
                    </p>
                  </div>
